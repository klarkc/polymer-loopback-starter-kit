...
* Backend server with [Ĺoopback](http://loopback.io/)

## Getting Started

To take advantage of Polymer Loopback Starter Kit you need to:
...
[Download](https://github.com/klarkc/polymer-loopback-starter-kit/releases/latest) and extract Polymer Loopback Starter Kit to where you want to work.

The standard version of Polymer Loopback Starter Kit comes with tools that are very handy when developing a Polymer app. If you'd like to get started without installing any new tools, see Polymer Loopback Starter Kit Light in the [Releases](https://github.com/klarkc/polymer-loopback-starter-kit/releases/latest) page.

### Install dependencies

#### Quick-start (for experienced users)

With Node.js installed, run the following one liner from the root of your Polymer Loopback Starter Kit download:

```sh
npm install -g gulp bower strongloop && npm install && bower install
```
...
3)  Install `gulp`, `bower` and `loopback` globally.

```sh
npm install -g gulp bower strongloop
```

This lets you run `gulp`, `bower` and `loopback` from the command line.

4)  Install the starter kit's local `npm` and `bower` dependencies.

```sh
cd polymer-loopback-starter-kit && npm install && bower install
```
...
The project is composed from multiple components.
 ...
 - `client/loopback/` provides an isomorphic loopback client with offline synchronization.
  The client needs some client-only models for data synchronization. These
  models are defined in `client/loopback/models/`.
  
 - `rest/` contains the REST API server; it exposes the shared models via
  REST API.

 - `client/` is the Polymer app project.

 - `server/` is the API server that brings the Loopback server components.
...
This outputs an IP address (for client and server) you can use to locally test and another that can be used on devices connected to your network.
...
This runs the unit tests defined in the `client/test` directory through [web-component-tester](https://github.com/Polymer/web-component-tester).
...
Polymer 1.0 introduces a shim for CSS custom properties. We take advantage of this in `client/styles/app-theme.html` to provide theming for your application. You can also find our presets for Material Design breakpoints in this file.

[Read more](https://www.polymer-project.org/1.0/docs/devguide/styling.html) about CSS custom properties.

## Unit Testing

Web apps built with Polymer Loopback Starter Kit come configured with support for [Web Component Tester](https://github.com/Polymer/web-component-tester) - Polymer's preferred tool for authoring and running unit tests. This makes testing your element based applications a pleasant experience.
...
The instructions assume the name of the new model is 'MyModel'.
 ...
 1. Create a file `common/models/my-model.json`, put the model definition there.
  Use `common/models/todo.json` as an example, see
  [loopback-boot docs](http://apidocs.strongloop.com/loopback-boot) for
  more details about the file format.

 2. (Optional) Add `common/models/my-model.js` and implement your custom model
  methods. See `common/models/todo.js` for an example.
 ...
 4. Define a client-only model to represent the remote server model in the
  client - create `client/loopback/models/my-model.json` with the following content:
 ...
 5. Add two entries to `client/loopback/models.json` to configure the new models
  for the client:
...
To enable Service Worker support for Polymer Loopback Starter Kit project use these 3 steps:
...
[(gulpfile.js)](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/gulpfile.js)
...
* For all other issues, feel free to file them [here](https://github.com/klarkc/polymer-loopback-starter-kit/issues).
...
If for any reason you need to disable Service Worker support after previously enabling it, you can remove it from your Polymer Loopback Starter Kit project using these 4 steps:

1. Remove references to the platinum-sw elements from your application [index](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/index.html).
2. Remove the two Platinum Service Worker elements (platinum-sw/..) in [client/elements/elements.html](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/elements/elements.html)
3. Remove 'precache' from the list in the 'default' gulp task ([gulpfile.js](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/gulpfile.js))
4. Navigate to `chrome://serviceworker-internals` and unregister any Service Workers registered by Polymer Loopback Starter Kit for your app just in case there's a copy of it cached.

## Yeoman support

[generator-polymer](https://github.com/yeoman/generator-polymer/releases) now includes support for Polymer Loopback Starter Kit out of the box.

## Frequently Asked Questions

### Where do I customise my application theme?

Theming can be achieved using [CSS Custom properties](https://www.polymer-project.org/1.0/docs/devguide/styling.html#xscope-styling-details) via [client/styles/app-theme.html](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/styles/app-theme.html).
You can also use `client/styles/main.css` for pure CSS stylesheets (e.g for global styles), however note that Custom properties will not work there under the shim.
...
This can be done via [`client/elements/routing.html`](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/elements/routing.html). We use Page.js for routing and new routes
can be defined in this import. We then toggle which `<iron-pages>` page to display based on the [selected](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/index.html#L105) route.
...
You can select a template and copy over the relevant parts you would like to reuse to Polymer Loopback Starter Kit.

You will probably need to change paths to where your Iron and Paper dependencies can be found to get everything working.
This can be done by adding them to the [`elements.html`](https://github.com/klarkc/polymer-loopback-starter-kit/blob/master/client/elements/elements.html) import.
...
`npm install` to see if this corrects the problem. If not, please check the [issue tracker](https://github.com/klarkc/polymer-loopback-starter-kit/issues) in case
...
There is also an [in-flight](https://github.com/klarkc/polymer-loopback-starter-kit/issues/62#issuecomment-108974016) issue
...
At the bottom of `client/index.html`, you will find a build block that can be used to include additional
...
Don't worry! We've got your covered. Polymer Loopback Starter Kit tries to offer everything you need to build and optimize your apps for production, which is why we include the tooling we do. We realise however that our tooling setup may not be for everyone.

If you find that you just want the simplest setup possible, we recommend using Polymer Loopback Starter Kit light, which is available from the [Releases](https://github.com/klarkc/polymer-loopback-starter-kit/releases) page. This takes next to no time to setup.

## Contributing

Polymer Loopback Starter Kit is a new project and is an ongoing effort by the Web Component community. We welcome your bug reports, PRs for improvements, docs and anything you think would improve the experience for other Polymer developers.
